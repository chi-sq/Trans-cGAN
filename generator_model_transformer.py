...
from blocks import Self_Attn, Multi_head_Self_Attn, SE_Block, Attention_Gate, Transblock
...
class Generator(nn.Module):
    def __init__(self, in_channels=3, features=64):
        ...
        #                                        num_heads=8, attn_drop_ratio=0.1, proj_drop_ratio=0.1)
        # Transblock
        self.trans1 = Transblock(in_dim=features*8, num_heads=8, attn_drop_ratio=0.1, proj_drop_ratio=0.1)
        self.trans2 = Transblock(in_dim=features*8, num_heads=8, attn_drop_ratio=0.1, proj_drop_ratio=0.1)
        self.trans3 = Transblock(in_dim=features * 8*2, num_heads=8, attn_drop_ratio=0.1, proj_drop_ratio=0.1)
        self.trans4 = Transblock(in_dim=features * 8*2, num_heads=8, attn_drop_ratio=0.1, proj_drop_ratio=0.1)
        ...
        # se_block
        # self.se_1024 = SE_Block(features * 16)
        # self.se_512 = SE_Block(features * 8)
        # self.se_256 = SE_Block(features * 4)
        # self.se_128 = SE_Block(features * 2)
        # Attention_Gate
    ...
    def forward(self, x):
        ...
        out = self.trans3((torch.cat([up1, d7], 1)))
        up2 = self.up2(out)
        # d6 = self.Att(up2, d6)
        ####### out, p_2 = self.Self_Attn4((torch.cat([up2, d6], 1)))
        out = self.trans4((torch.cat([up2, d6], 1)))
