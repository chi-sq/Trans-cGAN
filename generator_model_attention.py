...
from blocks import SE_Block, Attention_Gate
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        up2 = self.se(up2)
        # d6 = self.Att(up2, d6)
        up3 = self.up3(torch.cat([up2, d6], 1))  # 512
        up3 = self.se(up3)
        # d5 = self.Att(up3, d5)
        up4 = self.up4(torch.cat([up3, d5], 1))  # 512       #cattention_baseline只加了se在up4之后  _global所有层都加了se#
        # d4 = self.Att(up4, d4)
        up4 = self.se(up4)
